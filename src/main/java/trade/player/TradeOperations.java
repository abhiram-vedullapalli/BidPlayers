...
public class TradeOperations {
	...
	//function for Trade
		...
		public static LinkedList<Trade> requestsForMyPlayers(String teamName) {
			LinkedList<Trade> requestsToMe = new LinkedList<>();
			Filter playerFilter = new FilterPredicate("playerTeam" , FilterOperator.EQUAL , teamName);
		...
		public static LinkedList<Trade> requestsIMade(String teamName){
			LinkedList<Trade> requestsIMade = new LinkedList<>();
			Filter playerFilter = new FilterPredicate("ReqTeam" , FilterOperator.EQUAL , teamName);
			Query playerQuery = new Query("Trade").setFilter(playerFilter);
			PreparedQuery playerpq = ds.prepare(playerQuery);
			for(Entity players : playerpq.asIterable() ) {
				String playerName = (String) players.getProperty("playerName");
		...
		public static String acceptTradeProposal(String playerName , String reqTeamName , String offeringPrice) {
		
			
			//querying on Users kind for updating the balance and no of players bought
			Filter userFilter = new FilterPredicate("Team" , FilterOperator.EQUAL , reqTeamName);
			...
			if(offerPrice <= Balance) {
				...
				memcache.put(userKey.getName(), user);
				ds.put(user);
				...
				Filter ownerFilter = new FilterPredicate("Team" , FilterOperator.EQUAL , previousTeam);
				...
				memcache.put(owner.getKey().getName(), owner);
				ds.put(owner);
